...
sender_address = 'ameetreddy2001@gmail.com' #enter sender's email id
sender_pass = 'devy lnux mugq oivu' #enter sender's password
...
app.config['SECRET_KEY'] = 'ajsihh98rw3fyes8o3e9ey3w5dc'  # Set a secret key for session management

# Your routes and other configurations follow...
...
def voting():
    if request.method=='POST':
        ...
        recognizer.read(r"C:\Users\User\Desktop\pro\Smart-Voting-System-main\Trained.yml")
        cam=cv2.VideoCapture(0, cv2.CAP_DSHOW)
...
def select_candidate():
    ...
    ver = "SELECT verified FROM `voters` WHERE aadhar_id='"+aadhar+"'"
    # v = pd.read_sql_query(ver, mydb)
    ve= mydb.cursor()
    ve.execute(ver)
    v=ve.fetchone()
    ...
    if aadhar in all_adhar:
        ...
        if request.method == 'POST':
            ...
            headers = {
                "authorization": "scDmt6JUrMuZPwNa2EibphV4IxHWvzef8qg9O3QKCBL5AdnRFjFdpRZAxXyGfIozqgUeiDSHuwK7Cl68",
...
def admin_dashboard():
    if session.get('IsAdmin'):
        unverified_voters = pd.read_sql_query("SELECT * FROM voters WHERE verified='no'", mydb)
        return render_template('admin_verify.html', voters=unverified_voters)
    else:
        flash('Unauthorized access', 'danger')
        return redirect(url_for('home'))
    
@app.route('/admin_verify/<aadhar_id>', methods=['POST'])
def admin_verify(aadhar_id):
    if session.get('IsAdmin'):
        sql = "UPDATE voters SET verified='yes' WHERE aadhar_id=%s"
        cur = mydb.cursor()
        cur.execute(sql, (aadhar_id,))
        mydb.commit()
        cur.close()
        
        # Fetch the email of the voter to send verification email
        sql = "SELECT email FROM voters WHERE aadhar_id=%s"
        voter = pd.read_sql_query(sql, mydb, params=(aadhar_id,))
        if not voter.empty:
            email = voter['email'].values[0]
...
def send_verification_email(receiver_address):
    message = MIMEMultipart()
    message['From'] = sender_address
    message['To'] = receiver_address
    message['Subject'] = 'Verification Successful'
    mail_content = 'Your email has been successfully verified. You are now eligible to vote.'
